...
import { ProductConfiguratorSheet } from "@/components/storefront/product-configurator-sheet";
import { useCartStore } from "@/lib/ordering/cart-store";
import type { MenuItem, StorefrontFixture } from "@/lib/catalog/fixtures";
...
export function StorefrontMenu({ business, locale, initialTableNumber }: Props) {
  ...
  const orderingEnabled = business.orderingEnabled !== false;
                ...
                {section.items.map((item, i) => {
                  ...
                  return (
                    <MenuItemCard
                      ...
                      flash={isFlashing}
                      onAdd={
                        orderingEnabled && item.productId
                          ? () => {
                      ...
                      onDecrease={
                        orderingEnabled && item.productId && qty > 0
...
function hasCustomizations(item: MenuItem): boolean {
  return Boolean(item.hasConfiguration);
}
